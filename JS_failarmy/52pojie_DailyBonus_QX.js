...
$task.fetch(bonus).then(response => {
      ...
      $notify("52pojie Daily bonus", "", week[date.getDay()] + ", " + month[date.getMonth()] + " " + date.getDate() + ", " + "每日奖金成功！加油")
    } else {
      if (response.body.match(/\u4e0b\u671f\u518d\u6765/)) {
        $notify("52pojie Daily bonus", "", week[date.getDay()] + ", " + month[date.getMonth()] + " " + date.getDate() + ", " + "你已重复签到 ⚠️")
      } else {
